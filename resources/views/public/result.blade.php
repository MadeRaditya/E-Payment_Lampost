    ...
    <div class="bg-white border border-gray-100 shadow-sm rounded-xl p-6">
        <div class="space-y-3 mb-6">
            ...
            <div class="flex justify-between text-sm pt-3 border-t border-gray-100">
                ...
                <span class="font-bold text-red-600 text-lg">Rp {{ number_format($invoice->amount, 0, ',', '.') }}</span>
            </div>
        </div>

        @if($isPaid)
            <a href="#" class="block w-full text-center bg-red-600 hover:bg-red-700 text-white font-semibold py-3 rounded-lg transition">
                Download Kuitansi (Segera)
            </a>
        @else
